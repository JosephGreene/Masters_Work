function  Phase3D_G = MyMakingPhase3D_G_conv(Nx,Ny,Nz,lambda,deltaX,...
    deltaY,deltaZ,offsetZ,F,is_rm_alias,is_discr_fcn)
...
% propagation from all z-planes in the object to all other planes
...
if (is_discr_fcn)
    ...
    parfor j=1:Nz
        Phase3D_G(:,:,:,j) = Phase3D_G(:,:,:,j)...
            .* repmat(P,[1 1 Nz]) .* repmat(Q,[1 1 Nz]);
    end
end
