...
while(1)
    ...
    [z_est,~,E_rb_all,~] = forwardObj.A(xhatnext);
    resid_2D = z_est - Holo2D;

    %%% Update diagnistic information
    [cost_now,fid_cost_now,reg_cost_now] = evaluateCost2(resid_2D,xhatnext,tau,tvCost);
    %outs.cost(iIter+1) = cost_now; %% outs.cost(1)=cost_init
...
%     end
    ...
    if(cost_now < cost_prev)
        msg = sprintf('Reduced Cost Value = %2.10e\n\n',cost_now);
    elseif (cost_now > cost_prev)
        msg = sprintf('Increased Cost Value = %2.10e\n\n',cost_now);
    else
        msg = sprintf('Equal Cost Value = %2.10e\n\n',cost_now);
    end
